    ...
    <view class="category-level-box" v-if="cateList.length && isLoadingCate">
      ...
      <view class="category-goods">
        <scroll-view scroll-y="true" class="oneScroll">
          <view class="oneScroll-item using-hidden" v-for="(item, index) in cateList" :key="index"
        ...
        <view class="twoScroll">
          ...
          <scroll-view scroll-y="true" :style="{ height: 'calc(100% - ' + height + ' )' }">
            <image class="cate-adv" v-if="categoryAdvImage" :src="$util.img(categoryAdvImage)"
              @error="categoryAdvImage = $util.img('/upload/uniapp/default_ad.png')" mode="widthFix">
            </image>
            <view class="twoScroll-list" v-if="twoCateList.length">
              <view class="twoScroll-item" v-for="(item, index) in twoCateList" :key="index">
                <view v-if="item.child_list" @click="toListDetail(item)">
                  <image v-if="item.image" class="twoScroll-item-image" :src="$util.img(item.image)"
                    @error="cateImgError(index)" mode="aspectFit"></image>
                  <image v-else class="twoScroll-item-image" :src="$util.getDefaultImage().default_category_img"
                    mode="aspectFit"></image>
                  <text
                    class="using-hidden twoScroll-item-text">{{ item.short_name ? item.short_name : item.category_name }}</text>
                </view>
        ...
        <view class="category-list-wrap">
          ...
          <view>
            ...
            <view v-for="(item,index) in child_list" class="class-box" style="width: calc((100% - 30px) / 1)">
              <view style="width: calc((100% - 30px) / 1);background:#e7e7e7" @click="showCate(index)"
                v-if="item.child_list">
                {{item.category_name}}
                <span :class="[current.includes(index)?'iconfont iconiconangledown':'iconfont iconiconangledown-copy']"
                  style="font-size: 40rpx;"> 　</span>
              </view>
              <view style="width: calc((100% - 30px) / 1);background:#e7e7e7" @click="toDetail(item)" v-else>
                {{item.category_name}}
              </view>
              <!--四级分类-->
              <view v-for="(item2,index2) in item.child_list" style="width: calc((100% - 30px) / 1);background:#fafafa"
                @click="toDetail(item2)" v-show="current.includes(index)">
                {{item2.category_name}}
              </view>
            </view>
          </view>
          ...
          success: res => {
            ...
            if (res.code == 0 && res.data.length) {
              ...
              if (res.data[0].child_list) {
                ...
                if (res.data[0].child_list[0]) this.TwoCategoryId = res.data[0].child_list[0]
                  .category_id_2;
              }
