      ...
      <view class="goods-container">
          ...
          <view class="goods-img" :class="{ show: switchMedia == 'img' }">
            <swiper class="swiper" @change="swiperChange" :interval="swiperInterval" :autoplay="swiperAutoplay">
              <swiper-item v-for="(item, index) in goodsSkuDetail.sku_images" :key="index" :item-id="'goods_id_' + index">
                <view class="item" @click="previewMedia(index)"><image :src="$util.img(item, { size: 'big' })" @error="swiperImgError(index)" mode="aspectFit" /></view>
              </swiper-item>
            </swiper>
        ...
        <view @touchmove.prevent.stop class="videoPopup-box">
          <uni-popup ref="videoPopup" type="center">
            <view class="pop-video"><video :src="$util.img(goodsSkuDetail.video_url)" :poster="$util.img(goodsSkuDetail.sku_image, { size: 'big' })" objectFit="cover"></video></view>
          </uni-popup>
        </view>

        <!-- 限时折扣 -->
        <view v-if="showDiscount && goodsSkuDetail.discountTimeMachine" class="goods-promotion" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/goods/detail_promotion_left_bg.png') + ')' }">
        ...
        <view class="group-wrap padding-top">
          <view class="goods-module-wrap">
            ...
            <template v-else>
              <text v-if="is_wholesaler == 3" class="price color-base-text">{{ goodsSkuDetail.price_pf }}</text>
              <text v-else class="price color-base-text">请先认证</text>
            </template>
        ...
        <view class="group-wrap"><ns-fenxiao-goods-detail :skuId="skuId" ref="fenxiaoPopup"></ns-fenxiao-goods-detail></view>
            ...
            <uni-popup ref="couponPopup" type="bottom">
              <view class="goods-coupon-popup-layer popup-layer">
                ...
                <scroll-view class="coupon-body" scroll-y>
                  <view class="coupon-item ns-gradient-diy-goods-list" :class="{ disabled: item.is_lingqu != undefined }" v-for="(item, index) in couponList" :key="index" :data-theme="themeStyle">
                    <view class="coupon-info">
                      <view
                        class="info-wrap"
                        :style="{
                          backgroundImage: 'url(' + (item.is_lingqu != undefined ? $util.img('/upload/uniapp/coupon/bg_lingqu_gary.png') : $util.img('/upload/uniapp/coupon/bg_lingqu.png')) + ')',
                        }"
                      >
                      ...
                      <button type="primary" v-if="item.is_lingqu != undefined" @click="receiveCoupon(item, index)" disabled>已领取</button>
                      <button type="primary" v-else @click="receiveCoupon(item, index)">领取</button>
                    </view>
                  </view>
                </scroll-view>
              ...
              <view class="goods-wrap">
                <view class="goods-item" @click="toGoodsDetail(skuId)">
                  <view class="combo-img">
                    <image :src="$util.img(goodsSkuDetail.sku_image, { size: 'mid' })" @error="imgError()" />
                ...
                <block v-for="(item, index) in bundling[0].bundling_goods" :key="index">
                  <template v-if="index < 3">
                    <view class="goods-item" @click="toGoodsDetail(item.sku_id)">
                      <view class="combo-img">
                        <image :src="$util.img(item.sku_image, { size: 'mid' })" @error="bundlingImgError(0, index)" />
                        <view class="price-wrap">
                          <text class="unit">{{ $lang('common.currencySymbol') }}</text>
                            ...
                            <block v-for="(goods, goods_index) in item.bundling_goods" :key="goods_index">
                              <template v-if="goods_index < 3">
                                <view class="goods-item" @click="toGoodsDetail(goods.sku_id)">
                                  <view class="combo-img">
                                    <image :src="$util.img(goods.sku_image, { size: 'mid' })" @error="bundlingImgError(index, goods_index)" />
                                    <view class="price-wrap">
                                      <text class="unit">{{ $lang('common.currencySymbol') }}</text>
          ...
          <view class="goods-evaluate" @click="toEvaluateDetail(goodsSkuDetail.goods_id)">
            ...
            <view class="evaluate-item" v-if="goodsEvaluate.content">
              <view class="evaluator">
                <view class="evaluator-info">
                  <view class="evaluator-face">
                    <image v-if="goodsEvaluate.member_headimg" :src="$util.img(goodsEvaluate.member_headimg)" @error="goodsEvaluate.member_headimg = $util.getDefaultImage().default_headimg" mode="aspectFill" />
              ...
              <view class="evaluate-img" v-if="goodsEvaluate.images">
                <view class="img-box" v-for="(item, index) in goodsEvaluate.images" :key="index" @click="previewEvaluate(index, 'images')"><image :src="$util.img(item)" mode="aspectFit" /></view>
              </view>
            </view>
          </view>
        </view>
        ...
        <view class="goods-detail-tab">
          ...
          <view class="detail-content" :class="detailTab == 1 ? 'active' : ''">
            <view class="detail-content-item" v-show="detailTab == 0">
              <view class="goods-details" v-if="goodsSkuDetail.goods_content"><u-parse class="richText" :content="goodsSkuDetail.goods_content" @navigate="navigate"></u-parse></view>
